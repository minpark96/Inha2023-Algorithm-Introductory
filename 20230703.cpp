...
void HeapSort(int* ary, int maxIdx);
void MakeHeap(int* ary, int idx, int maxIdx);
...
int main()
{
	...
	cout << endl;
	cout << "Merge Sort - 비교: " << numCompMerge << "회, 교환: " << numSwapMerge << "회" << endl << endl;

	cout << "Heap Sort" << endl;
	HeapSort(ary7, size - 1);
	for (int i = 0; i < size; i++)
	{
		cout << ary7[i] << " ";
	}
	cout << endl;
	cout << "Heap Sort - 비교: " << numCompHeap << "회, 교환: " << numSwapHeap << "회" << endl;

	delete[] ary1, ary2, ary3, ary4, ary5, ary6, ary7;
...
void HeapSort(int* ary, int maxIdx)
{
	for (int i = maxIdx / 2; i >= 0; i--)
	{
		MakeHeap(ary, i, maxIdx);
	}

	for (int i = maxIdx; i > 0; i--)
	{
		Swap(ary[0], ary[maxIdx]);
		maxIdx--;
		MakeHeap(ary, 0, maxIdx);
	}
}
